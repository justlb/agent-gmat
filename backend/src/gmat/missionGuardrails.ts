...
export function validateGmatMissionGuardrails(template: GmatMissionGuardrailTemplate, values: MissionValues): MissionGuardrail[] {
  ...
  if (template === "electric-propulsion-transfer") {
    ...
    if (initialAltitude !== null && finalAltitude !== null && finalAltitude <= initialAltitude) guards.push(error("electric_altitude_order", "Electric-transfer target altitude must be greater than the initial altitude.", "transfer.finalAltitudeKm"))
