...
type OrbitKeepingFileKind = "digital-thread" | "ephemeris" | "log" | "manifest" | "opalis" | "report" | "result" | "rf-comlink" | "script" | "timeseries" | "values" | "vts"
...
async function listOrbitKeepingFiles(userWorkspaceRoot: string) {
  ...
  const files: Array<{ artifactId: string; fileName: string; historical?: boolean; kind: OrbitKeepingFileKind; mtimeMs: number; relativePath: string; runPath?: string; size: number }> = []
  const addMissionRunFiles = async (runsDir: string) => {
    ...
    for (const run of runs) {
      ...
      for (const artifact of await listRunArtifactHistory(runDir)) {
        const kind = orbitKeepingFileKind(path.basename(artifact.filePath))
        if (!kind) continue
...
export async function orbitKeepingRoutes(fastify: FastifyInstance, { config }: { config: AppConfig }) {
  ...
  fastify.post<{ Params: { draftId: string }; Body: DraftWorkspaceBody }>("/api/gmat/orbit-keeping/drafts/:draftId/execute", async (req, reply) => {
    ...
    try {
      ...
      const runPath = await finalizeMissionRun({ digitalThreadSnapshot, draftConversation: draft.conversation, result: result.result, root: userWorkspaceRoot, runDir: result.runDir, workspaceDir })
