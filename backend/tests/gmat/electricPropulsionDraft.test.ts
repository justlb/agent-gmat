...
describe("electric-propulsion transfer mission draft", () => {
  it("requires the Keplerian state and target altitude before confirmation", async () => {
    ...
    const updates = [
      ...
      ["spacecraft.dryMassKg", 850], ["spacecraft.initialFuelMassKg", 756], ["transfer.finalAltitudeKm", 1000],
    ...
    const targetSlot = values.slots.find(slot => slot.context.startsWith("targetFinalAltitudeKm ="))
    ...
    assert.ok(changes.some(change => change.id === targetSlot.id && change.value === "1000"))
