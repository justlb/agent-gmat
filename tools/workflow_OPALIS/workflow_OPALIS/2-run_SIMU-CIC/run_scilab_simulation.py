...
def normalize_path(path):
    ...
    # Scilab is a Windows executable. Its result marker contains C:/... even
    # when this Python worker runs in WSL; convert that return value back to a
    # filesystem path that Python can inspect and copy from.
    if os.name != "nt":
        forward = expanded.replace("\\", "/")
        if len(forward) >= 3 and forward[0].isalpha() and forward[1:3] == ":/":
            expanded = "/mnt/{0}/{1}".format(forward[0].lower(), forward[3:])
...
def scilab_path(path):
    ...
    # The runner may execute under WSL while Scilab is a Windows executable.
    # Scilab receives the launcher contents directly, so convert its mounted
    # Linux paths to Windows drive paths at this boundary only.
    if os.name != "nt" and normalized.startswith("/mnt/") and len(normalized) > 7:
        drive = normalized[5]
        if normalized[6] == "/" and drive.isalpha():
            normalized = drive.upper() + ":/" + normalized[7:]
    return normalized.replace("'", "''")


def scilab_command_path(path):
    """Convert a WSL-mounted path only when it is passed as a CLI argument
    to a Windows Scilab executable."""
    normalized = normalize_path(path)
    if os.name != "nt":
        forward = normalized.replace("\\", "/")
        if forward.startswith("/mnt/") and len(forward) > 7:
            drive = forward[5]
            if forward[6] == "/" and drive.isalpha():
                return drive.upper() + ":\\" + forward[7:].replace("/", "\\")
    return normalized
...
def command_for(executable, launcher, scihome, keep_open, gui):
    ...
    # WSL does not translate a /tmp argument for every Windows GUI program.
    # The launcher is stored in the mission directory and must be supplied to
    # WScilex as a real Windows drive path.
    launcher_argument = scilab_command_path(launcher) if os.name != "nt" and executable.lower().endswith(".exe") else launcher
    command.extend(["-f", launcher_argument])
    return command
...
def write_launcher(target_script, options, marker_file, error_marker_file):
    ...
    lines = [
        ...
        "CELESTLAB_DIR = {0};".format(scilab_string(scilab_path(options.celestlab_dir))),
...
def parse_args():
    ...
    parser.add_argument(
        "--celestlab-dir",
        required=True,
        help="Dossier CelestLab contenant loader.sce.",
    )
    parser.add_argument(
        "--save-root",
        required=True,
...
def main():
    ...
    if not os.path.isfile(args.base_scenario):
        print(
            "Erreur: scenario Simu-CIC introuvable: {0}".format(args.base_scenario),
            file=sys.stderr,
        )
        return 1
    if args.simucic_dir:
    ...
    if not os.path.isdir(os.path.join(args.simucic_dir, "lib")):
        ...
        return 1
    args.celestlab_dir = normalize_path(args.celestlab_dir)
    if not os.path.isfile(os.path.join(args.celestlab_dir, "loader.sce")):
        print(
            "Erreur: installation CelestLab invalide (loader.sce absent): {0}".format(
                args.celestlab_dir
            ),
            file=sys.stderr,
        )
    ...
    try:
        ...
        if os.path.isfile(marker_file):
            ...
            try:
                # Scilab writes a Windows C:/ path into the marker; this
                # worker may run in WSL and must inspect the mounted version.
                marked_path = marker.read().strip()
                result_dir = normalize_path(marked_path) if marked_path else None
            finally:
                marker.close()
