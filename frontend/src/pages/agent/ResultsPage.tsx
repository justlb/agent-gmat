...
function SelectedResults({ run, comparison, refresh }: { run: ResultRun; comparison?: ResultRun; refresh: string }) {
  ...
  return <>
    ...
    <details className="results-workflow-details"><summary>Pipeline details · {RESULT_STAGES.filter(([id]) => run.workflow?.stages[id]?.status === 'completed').length} / 4 stages completed</summary><section className="results-workflow" aria-label="Pipeline status">{RESULT_STAGES.map(([id, label]) => <div key={id}><strong>{label}</strong><span className={`results-status is-${run.workflow?.stages[id]?.status ?? 'unknown'}`}>{run.workflow?.stages[id]?.status.replaceAll('_', ' ') ?? 'Unknown'}</span>{run.workflow?.stages[id]?.message ? <small>{run.workflow.stages[id]?.message}</small> : null}</div>)}</section></details>
...
export function ResultsPage({ runPath, workspaceDir }: { runPath?: string; workspaceDir?: string | null }) {
  ...
  return <div className="results-layout">
    ...
    <main className="results-main" aria-label="Run results">{selected ? <SelectedResults key={selected.runPath} run={selected} comparison={comparison} refresh={selectedRefresh} /> : <div className="results-empty"><h2>{loading ? 'Loading mission runs…' : 'No saved run selected'}</h2><p>{error ? 'The run list could not be loaded. Use Refresh to retry.' : 'Select a saved run from the archive, including partial or failed calculations.'}</p></div>}</main>
