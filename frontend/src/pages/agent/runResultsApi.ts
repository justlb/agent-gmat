...
export const RESULT_STAGES = [
  ...
  ['simu_cic', 'Simu-CIC'],
  ['opalis', 'OPALIS'],
  ['rf_comlink', 'RF-COMLINK'],
] as const

export type ResultWorkflow = {
  stages: Partial<Record<(typeof RESULT_STAGES)[number][0], { status: string; message?: string | null }>>
}
...
export type ResultRun = {
  ...
  status: 'completed' | 'failed' | 'running' | 'partial' | 'not_started' | 'unknown'
...
async function request<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  const payload = await response.json().catch(() => {
    throw new Error(`The run service is temporarily unavailable (HTTP ${response.status}). Please retry.`)
  }) as T & { error?: string }
...
export function listResultRuns(workspaceDir?: string | null) {
  return request<{ runs: ResultRun[] }>(`/runs/results?${new URLSearchParams(workspaceDir ? { workspaceDir } : {})}`)
}

export function getResultConversation(runPath: string) {
  return request<{ conversation: ResultTurn[] }>(`/runs/conversation?${new URLSearchParams({ runPath })}`)
}

/** The API receives only a run path and question, keeping result discussion read-only. */
export function askResultAssistant(runPath: string, message: string) {
  return request<{ answer: string }>('/runs/analysis', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ runPath, message }),
  })
}
...
export async function getResultSamples(runPath: string) {
  ...
  return samples.flatMap(sample => {
    ...
    // Older electric-transfer artifacts have no altitude. This is a derived
    // semi-major-axis offset, not an instantaneous altitude for an ellipse.
    if (result.altitudeKm === undefined && result.semiMajorAxisKm !== undefined) {
      result.altitudeKm = result.semiMajorAxisKm - 6378.1363
    }
    return [result]
  })
}
