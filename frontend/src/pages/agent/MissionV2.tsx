...
const RESULT_ROWS = ['Lifetime', 'Fuel mass consumed', 'Average altitude', 'Contact time with GS', 'Latency (one-way / round-trip)', 'Eclipse time', 'Electrical configuration OK?']
const OVERVIEW_KEYS = ['lifetime', 'fuelMassConsumed', 'averageAltitude', 'contactTime', 'latency', 'eclipseTime', 'electricalConfiguration'] as const
const OVERVIEW_STAGES = ['gmat', 'gmat', 'gmat', 'simu_cic', 'simu_cic', 'simu_cic', 'opalis'] as const
...
export function MissionV2({ onStartMission, workspaceDir }: Props) {
  ...
  const overviewValue = (index: number) => {
    const key = OVERVIEW_KEYS[index]; const stage = pipeline[OVERVIEW_STAGES[index]]
    if (overview?.[key]) return overview[key].value
    if (stage.status === 'running') return `${OVERVIEW_STAGES[index] === 'gmat' ? 'GMAT' : OVERVIEW_STAGES[index] === 'simu_cic' ? 'Simu-CIC' : 'OPALIS'} running`
    if (stage.status === 'failed') return 'Unavailable'
    return `Waiting for ${OVERVIEW_STAGES[index] === 'gmat' ? 'GMAT' : OVERVIEW_STAGES[index] === 'simu_cic' ? 'Simu-CIC' : 'OPALIS'}`
  }
  return <div className="mission-v2-shell"><div className="mission-v2-grid">
    ...
    <main className="mission-v2-center">{satellite ? <section className="mission-v2-card"><span>SPACECRAFT DEFINITION · {satellite.id}@{satellite.version}</span><h2>{satellite.name}</h2><p>{satellite.description}</p><div className="mission-v2-facts"><b>Dry mass<br />{String(numberAt(physical, 'bus.physical.mass_kg.dry') ?? '—')} kg</b><b>Wet mass<br />{String(numberAt(physical, 'bus.physical.mass_kg.wet_at_launch') ?? '—')} kg</b><b>Propellant capacity<br />{String(numberAt(physical, 'bus.physical.mass_kg.propellant') ?? '—')} kg</b></div><dl className="mission-v2-definition"><dt>Drag model</dt><dd>{String(numberAt(physical, 'bus.physical.drag_area_m2') ?? '—')} m² · Cd {String(numberAt(physical, 'bus.physical.drag_coefficient') ?? '—')}</dd><dt>Propulsion</dt><dd>{String(numberAt(physical, 'bus.propulsion_subsystem.type') ?? '—')}</dd><dt>Specific impulse</dt><dd>{String(numberAt(physical, 'bus.propulsion_subsystem.specific_impulse_seconds') ?? '—')} s</dd><dt>Main thrust</dt><dd>{String(numberAt(physical, 'bus.propulsion_subsystem.main_engine_thrust_n') ?? '—')} N</dd><dt>Solar array</dt><dd>{String(numberAt(physical, 'bus.electrical_subsystem.solar_array.area_m2') ?? '—')} m² · {String(numberAt(physical, 'bus.electrical_subsystem.solar_array.maximum_power_w') ?? '—')} W</dd><dt>Battery</dt><dd>{String(numberAt(physical, 'bus.electrical_subsystem.battery.technology') ?? '—')} · {String(numberAt(physical, 'bus.electrical_subsystem.battery.capacity_wh') ?? '—')} Wh</dd><dt>Bus load</dt><dd>{String(numberAt(physical, 'bus.electrical_subsystem.spacecraft_bus_load_kw') ?? '—')} kW</dd><dt>Reference orbit</dt><dd>SMA {String(numberAt(physical, 'orbit.keplerian_elements.semi_major_axis_km') ?? '—')} km · ECC {String(numberAt(physical, 'orbit.keplerian_elements.eccentricity') ?? '—')} · INC {String(numberAt(physical, 'orbit.keplerian_elements.inclination_deg') ?? '—')}°</dd></dl><button type="button">Download complete reference JSON</button></section> : null}</main>
    <aside className="mission-v2-results"><section className="mission-v2-card mission-v2-overview"><span>KEY MISSION RESULTS</span><h2>Mission overview</h2><p>Live engineering indicators for this run.</p><div className="mission-v2-metrics">{RESULT_ROWS.map((row, index) => <div key={row} className={`metric-${pipeline[OVERVIEW_STAGES[index]].status}`}><label>{row}</label><b title={overview?.[OVERVIEW_KEYS[index]]?.source}>{overviewValue(index)}</b><small>{pipeline[OVERVIEW_STAGES[index]].status.replaceAll('_', ' ')}</small></div>)}</div></section><section className="mission-v2-card mission-v2-files"><span>GENERATED FILES</span><h2>Run artifacts</h2>{artifacts.length ? artifacts.map(file => <div key={file.id}><b>{file.relativePath.split('/').at(-1)}</b><small>{file.tool} · {Math.max(1, Math.round(file.size / 1024))} KB</small></div>) : <small>Files appear here as each tool completes.</small>}</section></aside>
  </div></div>
}
