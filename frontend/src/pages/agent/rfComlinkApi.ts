...
/** Runs, saves, extracts, and closes RF-COMLINK for this specific mission run. */
export async function runRfComlinkCalculation(runPath: string) {
  const response = await fetch(joinApiPath(undefined, "/rf-comlink/run"), {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ runPath }),
  })
  if (!response.ok) throw new Error(await responseError(response))
  return response.json() as Promise<{ ok: true; reportCount: number; scenario: string; summary: string }>
}
