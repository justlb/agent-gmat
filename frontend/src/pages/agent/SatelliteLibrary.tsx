...
export function SatelliteLibrary({ workspaceDir, onSelected }: { workspaceDir?: string | null; onSelected?: () => void }) {
  ...
  return <div className="satellite-library">
    ...
    <div className="satellite-library-grid">
      ...
      <aside className="satellite-active-card"><span>ACTIVE SATELLITE</span>{active ? <><small>{active.id}@{active.version}</small><h3>{active.name}</h3><hr /><h4>Orbit & mass</h4><dl><div><dt>Reference frame</dt><dd>{value(active, 'orbit.reference_frame')}</dd></div><div><dt>Mean altitude</dt><dd>{value(active, 'orbit.operational_parameters.altitude_km.mean', 'km')}</dd></div><div><dt>Dry / wet mass</dt><dd>{value(active, 'bus.physical.mass_kg.dry', 'kg')} / {value(active, 'bus.physical.mass_kg.wet_at_launch', 'kg')}</dd></div><div><dt>Propellant</dt><dd>{value(active, 'bus.physical.mass_kg.propellant', 'kg')} max</dd></div></dl><h4>Propulsion & power</h4><dl><div><dt>Propulsion</dt><dd>{value(active, 'bus.propulsion_subsystem.type')}</dd></div><div><dt>Specific impulse</dt><dd>{value(active, 'bus.propulsion_subsystem.specific_impulse_seconds', 's')}</dd></div><div><dt>Solar maximum</dt><dd>{value(active, 'bus.electrical_subsystem.solar_panels.total_power_generated_watts', 'W')}</dd></div><div><dt>Thruster power</dt><dd>{value(active, 'bus.propulsion_subsystem.electric_thruster.minimum_usable_power_kw', 'kW')}–{value(active, 'bus.propulsion_subsystem.electric_thruster.maximum_usable_power_kw', 'kW')}</dd></div></dl></> : <p>Choose a reference satellite to make it the physical source of truth for this workspace.</p>}</aside>
    </div>
  </div>
}
