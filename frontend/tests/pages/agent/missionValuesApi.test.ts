...
describe('updateMissionValue', () => {
  it('returns the draft type requested by the caller', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('/api/gmat/mission-values', expect.objectContaining({ method: 'PATCH' }))
  })

  it('rejects successful responses that omit the updated draft', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response(JSON.stringify({}), { status: 200 })))

    await expect(updateMissionValue({ path: 'initialOrbit.smaKm', template: 'orbit-keeping', value: '7000' })).rejects.toThrow('Unable to update mission value')
  })
})
